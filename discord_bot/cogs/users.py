...
class JoinTournamentButton(discord.ui.View):
    ...
    async def join_pk_db(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        logger.info(f"{interaction.user.id} -- {interaction.id} -- {interaction.data} -- {interaction.user.avatar.url}")
        await self.create_db_connect()
        await self.db_conn.execute('INSERT INTO users(discord_name, discord_id, bankroll, avatar_url) VALUES ($1, $2, $3, $4)', interaction.user.name, interaction.user.id, 0, interaction.user.avatar.url)
